...
** Worktree teardown  (kill buffers, then remove)
   ...
   Kill the worktree's buffers *before* =git worktree remove= — once the
   directory is gone, =projectile-kill-buffers= walks up to the next
   enclosing =.git= and scopes too broadly.

   | step | key / command                       | note                          |
   |------+-------------------------------------+-------------------------------|
   | 1    | C-c p k (projectile-kill-buffers)   | from a buffer in the worktree |
   | 2    | git worktree remove <wt>            | shell / magit                 |
   | 3    | M-x projectile-remove-known-project | or auto-cleanup next session  |
