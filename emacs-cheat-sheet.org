...
* Completion and Navigation
  ...
  Built-in =project.el= owns the =C-x p ...= command map (replaced
  projectile).
...
** Recent files & known projects  (manage the lists)
   ...
   Two separate lists: =recentf= tracks recent /files/; project.el's
   known-project list is what =C-x p p= offers. Most pruning now
   happens automatically (recentf on mode start, project.el zombie
   cleanup at startup), but to manage by hand:

   | command                            | what it does                          |
   |------------------------------------+---------------------------------------|
   | M-x recentf-edit-list              | mark recent /files/ to remove         |
   | M-x recentf-cleanup                | drop non-existent / excluded files    |
   | M-x project-forget-project         | drop one project from C-x p p         |
   | M-x project-forget-zombie-projects | drop projects whose dir is gone       |
   | C-h v recentf-list / project--list | inspect either list raw (the latter is internal) |
...
** Worktree teardown  (kill buffers, then remove)
   ...
   project.el can walk up to the next enclosing =.git= and scope too broadly.
   ...
   | 3    | M-x project-forget-project               | or let next-session zombie cleanup remove it |
...
** Rust (daily feedback loop)
   ...
   Reading source via rust-analyzer (Eglot wires these into the standard
   xref/eldoc keys — no Rust-specific bindings).  Jumping into stdlib source
   such as =unwrap_or_else= requires the =rust-src= component installed above.

   | key / command | result |
   |---------------+--------|
   | M-. | jump to the definition at point; on a stdlib symbol like =unwrap_or_else= this opens its source in =rust-src= |
   | M-, | pop back to where =M-.= jumped from |
   | M-? | find all references to the symbol at point |
   | C-h . | show the eldoc signature / hover doc for the symbol at point in the echo area |
   | M-x eldoc-doc-buffer | open the full rust-analyzer hover documentation in a dedicated buffer |
   | C-. | Embark on the symbol: xref, occur, rename-symbol, … |

   | key / command | result |
   |---------------+--------|
   | C-x p c | run =project-compile=; from a =.rs= buffer the prompt pre-fills =cargo check= (single crate) — edit to =cargo check --workspace= for a workspace, or =cargo test= to run tests |
   | M-x compile from Dired or a buffer at the Cargo root | run the same commands when the VCS project root is above the intended Cargo workspace |
   | g in the Compilation buffer / M-x recompile | repeat the last Cargo command with the same root |
   | C-x \` / M-g n | visit the next Cargo compiler location |
   | M-g p | visit the previous Cargo compiler location |
   | RET on a Compilation location | visit that exact compiler location |
   | C-c C-k | stop a running Cargo compilation |
   | C-c r f | format the current Rust buffer through Rustfmt via Eglot |
   | C-c ! n / C-c ! p | move through current Flymake (including rust-analyzer) diagnostics |
   | C-c ! l | show the Flymake diagnostic list |

   This is intentionally not format-on-save.  Keep formatting policy in the
   Cargo project's =rustfmt.toml= / =.rustfmt.toml=.  Use =cargo fmt --check=
   for a single crate, or =cargo fmt --all --check= for a workspace.  Run
   =cargo test= / =cargo test --workspace= through =C-x p c= when executing
   tests is wanted; it is deliberately separate from the fast =cargo check=
   loop.

   Cargo's default human-readable output stays enabled: the configuration
   recognizes its =--> file:line:column= locations in Compilation buffers.
   The =cargo check= pre-fill is a buffer-local =compile-command= set in Rust
   source buffers, so it appears only when =C-x p c= is invoked from a =.rs=
   buffer; from Dired or a non-Rust buffer the prompt still defaults to
   =make -k=.  =project-compile= is correct when the Emacs project root is the
   intended Cargo root.  If a repository contains multiple Cargo workspaces, use
   =cargo locate-project --workspace= to identify the desired manifest, open
   its directory in Dired, and run =M-x compile= there.
...
** Eshell
   ...
   | C-x p e     | open Eshell at the project root (project-eshell)         |
...
** project.el
    ...
    Built-in project commands live on the =C-x p= prefix
    (=project-prefix-map=); =C-x p C-h= lists them all.

    | key     | description                                |
    |---------+--------------------------------------------|
    | C-x p p | switch project, then pick a command        |
    | C-x p f | find file in project                       |
    | C-x p b | switch to a buffer in project              |
    | C-x p D | run dired at the project root              |
    | C-x p k | kill project buffers                       |
    | C-x p e | open Eshell at the project root            |
    | C-x p c | compile at the project root                |
