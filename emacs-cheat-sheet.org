...
** Rust (daily feedback loop)
   ...
   | key / command | result |
   |---------------+--------|
   | C-x p c | run =project-compile= at the project root; enter =cargo check= for a single crate or =cargo check --workspace= for a workspace |
   ...
   =project-compile= is correct when the Emacs project root is the intended
   Cargo root.  If a repository contains multiple Cargo workspaces, use
