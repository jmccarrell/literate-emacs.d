...
*** Next Up
   ...
   - [ ] re-enable the use of counsel by reducing zsh shell load time
   - [ ] add the hydra so I can grow the swiper buffer

*** General

    - Explore the three shell modes in the play emacs like an instrument video
...
*** Reduce Startup Time for zsh
    ...
    I find the battle to keep my shell startup time low never ending.  I have come to regard it in the same
    vein as spring cleaning or maybe dental visits for teeth cleaning: useful, but never my first choice.

    As of this moment, my interactive zsh startup times are on the order of 1 second
    ...
    which is too long.  [[https://github.com/abo-abo/swiper/issues/2190#issuecomment-525385066][Especially for counsel]].  So configure the arguments given to zsh by emacs when
    attempting to run processes via =shell-command= and friends to ignore reading initialization files.
    That this works is dependent on already having a reasonable environment configured, including PATH.
...
**** What is =-conorcs=?
     ...
     - =-c=
     - =-o norcs=
     and as a single argument to reduce potential issues with whitespace separation.  Express that as
     =-conorcs=.
...
**** Effect
    ...
    After this setting is in place, response time for emacs executed processes improve.
