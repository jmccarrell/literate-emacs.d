...
** gptel
   ...
   First landing: a plain chat client with one cloud backend.  The
   current local default is an Ollama-backed Qwen MLX model, while
   Claude and Gemma 4 remain selectable. Tools, MCP, agentic editing,
   and inline completion stay as separate follow-up sub-goals.

   Store the Anthropic API key in auth-source, for example:

   #+begin_example
     machine api.anthropic.com login apikey password sk-ant-...
   #+end_example

   For the local backends, keep Ollama running with the Qwen MLX
   default and Gemma 4 fallback available:
