...
** Use-Package
   ...
   =use-package= is built in to Emacs since 29, so the bootstrap install
   is gone. We still set the two defaults and =require= it so the macro
   is fully loaded at config-parse time.
