...
** gptel
   ...
   first local follow-up makes a single Ollama-backed Gemma 4 model
   the default, while keeping Claude selectable and keeping tools,
   MCP, agentic editing, and inline completion as separate follow-up
   sub-goals.
   ...
   For the local backend, keep Ollama running with =gemma4:26b=
   available:
