...
** Adjust font size to display resolution
   ...
   On my 2021 MacBook with retina display at maximum resolution (2056x3129),
   I wind up with these textsize values:
   #+begin_example
     monitor size  mm: 344
     monitor size pix: 2056
     pixel pitch 0.17
     textsize default points 15
     textsize frame offset 0
     pixel pitch adjustment 0
     monitor size adjustment 0
     text size chosen: 15
     default-font-width 9
     default-font-height 18
   #+end_example

   so choose the default monitor size offset (0) in the usual case of running
   Emacs on the retina display.
